...
describe('Places', () => {
  ...
  describe('GET /api/places/:id', () => {
    ...
    test('it should 200 and return the requested place', async () => {
      const response = await request.get(`${url}/${data.places[0].id}`).set('Authorization', loginHeader);
      expect(response.status).toBe(200);
  ...
  describe('DELETE /api/places/:id', () => {
    ...
    test('it should 204 and return nothing', async () => {
      const response = await request.delete(`${url}/${data.places[0].id}`).set('Authorization', loginHeader);
